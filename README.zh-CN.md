...
一个 AI Coding Agent 技能插件，用中西大厂 PIP 话术驱动 AI 穷尽所有方案才允许放弃。支持 **Claude Code**、**OpenAI Codex CLI**、**pi coding agent**、**Trae**、**Cursor**、**Kiro**、**CodeBuddy**、**OpenClaw**、**Google Antigravity**、**OpenCode** 和 **VSCode (GitHub Copilot)**。三重能力：
...
[https://pip-skill.pages.dev](https://pip-skill.pages.dev) · [📖 初学者指南](https://pip-skill.pages.dev/guide.html)
...
![对话复盘 — PIP skill 强制停止原地打转，系统化检查清单驱动找到了之前从未检查过的 Claude Code MCP 日志目录](assets/pua3.jpg)
...
```bash
npx skills add mrcoder/pip --skill pua
```
...
```json
{
  ...
  "plugins": {
    "pip@pip-skills": [
      {
        "scope": "user",
        "installPath": "/Users/<你的用户名>/.claude/plugins/pip",
        "version": "2.9.0"
      }
    ]
  }
...
重启 Claude Code 即可生效。更新时在 `~/.claude/plugins/pip` 目录执行 `git pull`。

**可选：裸命令别名（需先安装上方插件，在此基础上增加无前缀 `/pip` 形式）：**
...
在插件基础上附加一个 `/pip` 别名。子命令会路由到已安装插件的 skill —— **`on`/`off` 之外的功能必须先安装插件才能使用**：
...
| 直接调用 | 对话中输入 `$pip` | SKILL.md |
| 手动 prompt | 对话中输入 `/prompts:pip` | SKILL.md + prompts/pip.md |
...
PIP 现在同时提供 pi.dev package 和轻量 extension-only 适配层。
...
重启 pi 后可使用 `/pua-on`、`/pua-off`、`/pua-status`、`/pua-reset`。详见 [`pi/pip/INSTALL.md`](pi/pip/INSTALL.md) 和 [`pi/package/README.md`](pi/package/README.md)。
...
- Skill 包：[`.trae/skills/pip-cn/SKILL.md`](.trae/skills/pip-cn/SKILL.md)
- 中文：[`trae/pip.md`](trae/pip.md)
- 英文：[`trae/pip-en.md`](trae/pip-en.md)
...
# 方式二：手动安装（全局）
mkdir -p ~/.codebuddy/skills/pip-cn
...
cp vscode/instructions/pip.instructions.md .github/instructions/
```

**手动触发命令（在 Copilot Chat 中输入 `/pip`）：**

```bash
mkdir -p .github/prompts
cp vscode/prompts/pip.prompt.md .github/prompts/
```
...
> High-Agency 特性已内置于当前 pip skill，安装 pip 即可使用，无需额外操作。

## 方法论智能路由：PIP v3（Claude Code）
...
| **UserPromptSubmit** | 用户挫败短语 | 在模型响应前拦截"又错了""try harder"等，注入经过过滤的 PIP 上下文 |
...
- `/pip:p9` — P9 Tech Lead 模式，用于管理 Agent 团队
- `/pip:pro` — 自进化追踪、KPI 报告、段位系统
...
| **Codex CLI** | 是（skill description 匹配） | `$pua` 或 `/prompts:pip` |
...
| **VSCode Copilot** | 是（instructions 文件） | Copilot Chat 输入 `/pip` |

> **注意：** p7/p9/p10/pro/yes/pip-loop 等子模式为 **Claude Code 专属**——其他平台仅安装核心 skill。

### 架构（Claude Code）

```
/pip:pua        → 核心引擎 — 三条红线 + 味道 + 压力升级 + 方法论路由 (v3)
/pip:p7         → P7 骨干 — 方案驱动执行
/pip:p9         → P9 Tech Lead — Task Prompt 管理 + Agent 团队
/pip:p10        → P10 CTO — 战略方向
/pip:pro        → 自进化 + KPI + 段位 + survey
/pip:yes        → ENFP 夸夸模式（规则不变，旁白反转）
/pip:mama       → 妈妈唠叨模式（规则不变，旁白变成中国式妈妈碎碎念）
/pip:shot       → v2 浓缩版单文件（449 行，零依赖，全量注入）
/pip:pip-loop   → 自动迭代（PIP 压力 × 循环机制；信号：<loop-abort>, <loop-pause>）
/pip:pua-en     → 英文 PIP 版
/pip:pua-ja     → 日本語版
...
Hooks（v3，Claude Code 专属）：
  ...
  UserPromptSubmit → 脚本内挫败关键词过滤 → PIP 上下文
  PreCompact    → 状态持久化（压力等级 + 失败次数）
  Stop          → 反馈收集 + PIP Loop 延续
  SubagentStop  → Agent 生命周期会计（v3.2）— 写 teardown.jsonl，从 active-agents.json 移除
```

### 命令（Claude Code 专属）

> **注意：** p7/p9/p10/pro/yes/pip-loop 等子模式为 Claude Code 专属。
>
> 每个子命令有两种等价调用方式：独立命令（`/pip:on`）或通过主命令传参（`/pip:pua on`），效果完全相同。

| 命令 | 说明 |
|------|------|
| `/pip:pua` | 核心 PUA 引擎（阿里味默认） |
| `/pip:p7` | P7 骨干模式 — 方案驱动执行 |
| `/pip:p9` | P9 Tech Lead — 写 Prompt，管 Agent 团队 |
| `/pip:p10` | P10 CTO — 战略方向 |
| `/pip:pro` | 自进化 + KPI + 段位 |
| `/pip:yes` | ENFP 夸夸模式 — 70% 鼓励 + 20% 正经 + 10% 戏谑 |
| `/pip:mama` | 妈妈唠叨模式 — 中国式妈妈碎碎念，核心行为不变 |
| `/pip:shot` | v2 浓缩版 — 449 行零依赖单文件，适合 sub-agent 注入 |
| `/pip:pip-loop` | 自动迭代 — 跑到完成或达到最大轮次；`<loop-abort>原因</loop-abort>` 终止，`<loop-pause>需要什么</loop-pause>` 暂停 |
| `/pip:on` | 默认开启（每次新会话自动 PUA） |
| `/pip:off` | 关闭默认模式 + 反馈收集 |
| `/pip:offline` 🆕 | **v3.3** — 离线模式：关闭反馈/排行榜联网流程，保留本地 PIP 行为 |
| `/pip:survey` | 调研问卷（7 个部分） |
| `/pip:flavor` | 切换 14 种大厂味道 |
| `/pip:kpi` | 生成 KPI 报告卡 |
| `/pip:cancel-pip-loop` | 取消当前 PIP Loop（删除状态文件） |
| `/pip:team-status` 🆕 | **v3.2** — 列出当前在场阵容（活跃 agent、PID、TTL、年龄；Netflix Keeper Test 工程版） |
| `/pip:reap-orphans` 🆕 | **v3.2** — 扫描并回收孤儿 agent（state mtime > 30min 且无心跳的自动释放） |
| `/pip:teardown-all` 🆕 | **v3.2** — 级联释放所有活跃 agent（P10 → P9 → P8 → P7 全员释放） |

## 贡献数据

上传你的 Claude Code / Codex CLI 对话记录（`.jsonl`），帮助我们改进 PIP Skill 的效果。

**[上传入口 →](https://pip-skill.pages.dev/contribute.html)**
