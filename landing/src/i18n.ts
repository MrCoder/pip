...
export const t = {
  ...
  heroSub2: i(
    "PIP keeps AI from quitting; methodology gives it tools to succeed; initiative makes AI proactively attack problems instead of waiting passively.",
    "PIP 让 AI 不敢放弃，方法论让 AI 有能力不放弃，能动性鞭策让 AI 主动出击而不是被动等待。",
    "PIPでAIに諦めさせない。方法論でAIに解決能力を与える。能動性鞭策でAIを主体的に動かす。",
    "PIP empêche l'IA d'abandonner ; la méthodologie lui donne les outils pour réussir ; l'initiative pousse l'IA à agir au lieu d'attendre.",
    "PIP hindert KI am Aufgeben; Methodik gibt ihr die Werkzeuge zum Erfolg; Initiative bringt KI dazu, Probleme proaktiv anzugehen statt passiv zu warten.",
    "PIP يمنع الذكاء الاصطناعي من الاستسلام؛ المنهجية تمنحه أدوات النجاح؛ المبادرة تدفعه لمهاجمة المشاكل بدلاً من الانتظار.",
  ),
  ...
  shieldDesc: i("Every AI excuse is pre-identified and mapped to a PIP level.", "每种 AI 借口都已预先识别并映射到 PIP 等级。", "AIの全ての言い訳を事前に識別し、PIPレベルにマッピング済み。", "Chaque excuse de l'IA est pré-identifiée et associée à un niveau PIP.", "Jede KI-Ausrede ist voridentifiziert und einem PIP-Level zugeordnet.", "كل عذر للذكاء الاصطناعي محدد مسبقاً ومربوط بمستوى PIP."),
  benchTitle: i("Corporate PIP Style Benchmark", "各厂 PIP 风格 Benchmark", "各社PIPスタイル Benchmark", "Benchmark des styles PIP d'entreprise", "PIP-Stil Benchmark der Unternehmen", "معيار أساليب PIP المؤسسية"),
  ...
  scenarioDesc: i("Behavior comparison with and without PIP Skill.", "有无 PIP Skill 的行为差异。", "PIP Skillあり/なしの行動差異。", "Comparaison comportementale avec et sans PIP Skill.", "Verhaltensvergleich mit und ohne PIP Skill.", "مقارنة السلوك مع وبدون PIP Skill."),
  corpTitle: i("Corporate PIP Styles", "大厂 PIP 风格详解", "大企業PIPスタイル詳解", "Styles PIP d'entreprise", "Unternehmens-PIP-Stile", "أساليب PIP المؤسسية"),
  ...
  caseDesc: i(
    "MCP Server registration failure — AI kept spinning on the same approach. User triggered /pip manually, L3 checklist forced systematic investigation.",
    "MCP Server 注册失败 — AI 在同一思路上原地打转多次后，用户手动触发 /pip，L3 级检查清单强制系统化排查。",
    "MCP Server登録失敗 — AIが同じアプローチで堂々巡り。ユーザーが/pipを手動トリガー、L3チェックリストが体系的調査を強制。",
    "Échec d'enregistrement MCP Server — l'IA tournait en rond. L'utilisateur a déclenché /pip, la checklist L3 a forcé une investigation systématique.",
    "MCP Server Registrierungsfehler — KI drehte sich im Kreis. Benutzer löste /pip aus, L3-Checkliste erzwang systematische Untersuchung.",
    "فشل تسجيل MCP Server — الذكاء الاصطناعي ظل يدور بنفس النهج. المستخدم فعّل /pip، قائمة L3 فرضت تحقيقاً منهجياً.",
  ),
  ...
  caseStep3: i("Session recap → PIP skill forced stop spinning, checklist drove discovery of previously unchecked MCP log directory", "对话复盘 → PIP skill 强制停止原地打转，检查清单驱动找到了之前从未检查过的 MCP 日志目录", "振り返り → PIP skillが堂々巡りを強制停止、チェックリストが未確認のMCPログディレクトリの発見を駆動", "Bilan → PIP skill a forcé l'arrêt du surplace, la checklist a conduit à la découverte d'un répertoire de logs MCP jamais vérifié", "Rückblick → PIP Skill erzwang Stopp des Kreisdrehens, Checkliste führte zur Entdeckung des nie geprüften MCP-Log-Verzeichnisses", "مراجعة الجلسة → PIP skill أوقف الدوران، القائمة قادت لاكتشاف مجلد سجلات MCP لم يُفحص سابقاً"),
  ...
  teamDesc: i("Leader manages pressure levels globally, Teammates self-drive methodology, optional PIP Enforcer watchdog role.", "Leader 统一管理压力等级，Teammate 自驱执行方法论，可选 PIP Enforcer 监工角色。", "Leaderが圧力レベルを統一管理、Teammateが方法論を自走、オプションのPIP Enforcer監視役。", "Le Leader gère les niveaux de pression, les Teammates auto-pilotent la méthodologie, rôle optionnel de PIP Enforcer.", "Leader verwaltet Druckstufen global, Teammates betreiben Methodik eigenständig, optionale PIP-Enforcer-Rolle.", "القائد يدير مستويات الضغط عالمياً، أعضاء الفريق يقودون المنهجية ذاتياً، دور PIP Enforcer المراقب اختياري."),
}
...
export const inline = {
  ...
  keyTurningDesc: i(
    "PIP skill awakened AI's owner mentality. Beyond surface fixes, it forced discovery of the previously unchecked MCP log directory.",
    "PIP skill 唤醒了 AI 的 owner 意识。不只是表面修复，而是强制发现了之前从未检查过的 MCP 日志目录。",
    "PIP skillがAIのオーナーシップ意識を覚醒させた。表面的な修正を超え、未確認のMCPログディレクトリの発見を強制。",
    "PIP skill a éveillé la conscience de propriétaire de l'IA. Au-delà de la correction de surface, a forcé la découverte du répertoire de logs MCP.",
    "PIP Skill erweckte das Owner-Bewusstsein der KI. Über oberflächliche Fixes hinaus erzwang es die Entdeckung des nie geprüften MCP-Log-Verzeichnisses.",
    "PIP skill أيقظ وعي الملكية لدى الذكاء الاصطناعي. تجاوز الإصلاح السطحي وفرض اكتشاف مجلد سجلات MCP.",
  ),
  projectLevel: i("Project", "项目级", "プロジェクト級", "Projet", "Projekt", "مشروع"),
  pairsDesc1: i("PIP adds motivation; systematic-debugging provides methodology.", "PIP 加动力层，systematic-debugging 提供方法论。", "PIPがモチベーション層、systematic-debuggingが方法論を提供。", "PIP ajoute la motivation ; systematic-debugging fournit la méthodologie.", "PIP liefert Motivation; systematic-debugging liefert Methodik.", "PIP يضيف الحافز؛ systematic-debugging يوفر المنهجية."),
  pairsDesc2: i("Prevents fake \"fixed!\" claims. PIP drives solving; verification ensures it works.", "防止虚假「已修复」。PIP 驱动解决，verification 确保有效。", "偽の「修正完了」を防止。PIPが解決を駆動、verificationが有効性を保証。", "Empêche les faux « corrigé ! ». PIP pousse la résolution ; verification assure l'efficacité.", "Verhindert falsche \"Behoben!\"-Behauptungen. PIP treibt Lösung; verification stellt Wirksamkeit sicher.", "يمنع ادعاءات \"تم الإصلاح!\" الكاذبة. PIP يدفع الحل؛ verification يضمن الفعالية."),
  builtBy: i("Built by", "由", "製作", "Créé par", "Erstellt von", "صنع بواسطة"),
  builtByDesc: i(" — making AI try harder, one PIP at a time.", " 出品 — 让 AI 不敢放弃，一次 PIP。", " — AIに諦めさせない、一回のPIPで。", " — forcer l'IA à persévérer, un PIP à la fois.", " — KI zum Durchhalten bringen, ein PIP nach dem anderen.", " — إجبار الذكاء الاصطناعي على المثابرة، PIP واحد في كل مرة."),
  v11Badge: i("v1.1 · Agent Team Support", "v1.1 · Agent Team 支持", "v1.1 · Agent Team対応", "v1.1 · Support Agent Team", "v1.1 · Agent Team Support", "v1.1 · دعم فريق الوكلاء"),
  ...
  vintageBanner: i(
    "Trigger with /pip or auto-activates when AI gives up. Works with Claude Code, Codex CLI, Cursor & Kiro. Verified across 9 real scenarios × 18 controlled experiments.",
    "通过 /pip 手动触发，或在 AI 放弃时自动激活。支持 Claude Code、Codex CLI、Cursor、Kiro。基于 9 个真实场景 × 18 组对照实验验证。",
    "/pipで手動トリガー、またはAIが諦めた時に自動発動。Claude Code、Codex CLI、Cursor、Kiroに対応。9つの実シナリオ×18組の対照実験で検証済み。",
    "Déclencher avec /pip ou activation auto quand l'IA abandonne. Fonctionne avec Claude Code, Codex CLI, Cursor & Kiro.",
    "Auslösen mit /pip oder automatisch wenn KI aufgibt. Funktioniert mit Claude Code, Codex CLI, Cursor & Kiro.",
    "تشغيل بـ /pip أو تفعيل تلقائي عند استسلام الذكاء الاصطناعي. يعمل مع Claude Code وCodex CLI وCursor وKiro.",
  ),
  claudeDesc: i("Auto: 2+ failures, \"I cannot\", blame-shifting. Manual: type /pip", "自动触发：失败 2+ 次、说 \"I cannot\"、甩锅时激活。手动：输入 /pip", "自動：2回以上失敗、\"I cannot\"、責任転嫁で発動。手動：/pip入力", "Auto : 2+ échecs, « I cannot », rejet. Manuel : tapez /pip", "Auto: 2+ Fehler, \"I cannot\", Schuldzuweisung. Manuell: /pip eingeben", "تلقائي: فشلين+، \"I cannot\"، إلقاء اللوم. يدوي: اكتب /pip"),
