...
<p align="center">
  <img src="assets/hero.jpeg" alt="PIP Skill — 効率倍増" width="250">
</p>
...
[https://pip-skill.pages.dev](https://pip-skill.pages.dev) · [📖 初心者ガイド](https://pip-skill.pages.dev/guide.html)
...
![対話の振り返り — PIP skillが堂々巡りを強制停止、体系的チェックリストが以前チェックしたことのなかったClaude Code MCPログディレクトリの発見を促した](assets/pua3.jpg)

**キーとなる転換点：** PIP skillがAIに同じ思考での堂々巡り（プロトコル形式の変更、バージョン番号の推測）を強制停止させ、7項目チェックリストの実行に切り替えた。エラーメッセージを一字一句読む → Claude Code自身のMCPログディレクトリを発見 → `claude mcp` の登録メカニズムが手動の `.claude.json` 編集と異なることを発見 → 根本原因解決。
...
```bash
npx skills add mrcoder/pip --skill pua-ja
```
...
```json
{
  ...
  "plugins": {
    "pip@pip-skills": [
      {
        "scope": "user",
        "installPath": "/Users/<ユーザー名>/.claude/plugins/pip",
        "version": "2.9.0"
      }
    ]
  }
...
Claude Codeを再起動して反映。更新は `~/.claude/plugins/pip` で `git pull` を実行。

**オプション：ベアコマンドエイリアス（上記プラグインのインストールが必要 — プレフィックスなし `/pua` 形式を追加）：**

```bash
curl -o ~/.claude/commands/pua.md \
  https://raw.githubusercontent.com/mrcoder/pip/main/commands/pua.md
```
...
| `/pua pro` | `/pip:pro` |
| `/pua yes` | `/pip:yes` |
| `/pua loop` | `/pip:pip-loop` |
| `/pua kpi` | `/pip:kpi` |
| `/pua survey` | `/pip:survey` |
| `/pua flavor` | `/pip:flavor` |
...
```bash
mkdir -p ~/.codex/skills/pip-ja-ja
curl -o ~/.codex/skills/pip-ja-ja/SKILL.md \
  https://raw.githubusercontent.com/mrcoder/pip/main/codex/pua-ja/SKILL.md
...
| 手動プロンプト | 対話で `/prompts:pua` と入力 | SKILL.md + prompts/pip.md |

プロジェクトレベルインストール（現在のプロジェクトのみ有効）：

```bash
mkdir -p .agents/skills/pip-ja-ja
curl -o .agents/skills/pip-ja-ja/SKILL.md \
  https://raw.githubusercontent.com/mrcoder/pip/main/codex/pua-ja/SKILL.md
...
curl -o .cursor/rules/pua-ja.mdc \
  https://raw.githubusercontent.com/mrcoder/pip/main/cursor/rules/pua-ja.mdc
```
...
curl -o .kiro/steering/pua-ja.md \
  https://raw.githubusercontent.com/mrcoder/pip/main/kiro/steering/pua-ja.md
```

**方法2：Agent Skills（Claude Codeと同じ形式）**

```bash
mkdir -p .kiro/skills/pip-ja-ja
curl -o .kiro/skills/pip-ja-ja/SKILL.md \
  https://raw.githubusercontent.com/mrcoder/pip/main/skills/pip-ja-ja/SKILL.md
```
...
# 方法2：手動インストール（グローバル）
mkdir -p ~/.codebuddy/skills/pip-ja
...
```bash
# ClawHub経由でインストール
clawhub install pip-ja

# または手動インストール
mkdir -p ~/.openclaw/skills/pip-ja-ja
curl -o ~/.openclaw/skills/pip-ja-ja/SKILL.md \
  https://raw.githubusercontent.com/mrcoder/pip/main/skills/pip-ja-ja/SKILL.md
```
...
```bash
mkdir -p skills/pip-ja-ja
curl -o skills/pip-ja-ja/SKILL.md \
  https://raw.githubusercontent.com/mrcoder/pip/main/skills/pip-ja-ja/SKILL.md
```
...
mkdir -p ~/.gemini/antigravity/skills/pip-ja-ja
curl -o ~/.gemini/antigravity/skills/pip-ja-ja/SKILL.md \
  https://raw.githubusercontent.com/mrcoder/pip/main/skills/pip-ja-ja/SKILL.md
```
...
```bash
mkdir -p .agent/skills/pip-ja-ja
curl -o .agent/skills/pip-ja-ja/SKILL.md \
  https://raw.githubusercontent.com/mrcoder/pip/main/skills/pip-ja-ja/SKILL.md
```
...
```bash
# グローバルインストール（全プロジェクトで利用可能）
mkdir -p ~/.config/opencode/skills/pip-ja-ja
curl -o ~/.config/opencode/skills/pip-ja-ja/SKILL.md \
  https://raw.githubusercontent.com/mrcoder/pip/main/skills/pip-ja-ja/SKILL.md
```
...
```bash
mkdir -p .opencode/skills/pip-ja-ja
curl -o .opencode/skills/pip-ja-ja/SKILL.md \
  https://raw.githubusercontent.com/mrcoder/pip/main/skills/pip-ja-ja/SKILL.md
```
...
2回以上失敗したteammateはLeaderに[PIP-REPORT]形式で報告すること。
...
curl -o .claude/agents/pip-enforcer-ja.md \
  https://raw.githubusercontent.com/mrcoder/pip/main/agents/pua-enforcer-ja.md
```
...
| 永続的な共有変数なし | `[PIP-REPORT]`メッセージ形式で状態伝達 |
...
- `/pip:p9` — P9 Tech Leadモード — Agentチームの管理に
- `/pip:pro` — 自己進化追跡、KPIレポート、ランクシステム
...
> **注意：** p7/p9/p10/pro/yes/pip-loopなどのサブモードは**Claude Code専用**——他のプラットフォームはコアskillのみインストール。

### アーキテクチャ（Claude Code）

```
/pip:pua        → コアエンジン — 三鉄則 + フレーバー + プレッシャー + 方法論ルーター（v3）
/pip:p7         → P7 骨幹 — ソリューション駆動実行
/pip:p9         → P9 Tech Lead — Task Prompt管理 + Agentチーム
/pip:p10        → P10 CTO — 戦略方向
/pip:pro        → 自己進化 + KPI + ランクシステム + survey
/pip:yes        → ENFP 褒めモード（ルール不変、雰囲気反転）
/pip:pip-loop   → 自動反復（PUAプレッシャー × ループ機構；シグナル：<loop-abort>, <loop-pause>）
/pip:pua-en     → 英語PIP版
/pip:pua-ja     → 日本語版
...
Hooks（v3、Claude Code専用）:
  ...
  Stop          → フィードバック収集 + PIP Loop継続判定
  SubagentStop  → Agent ライフサイクル会計（v3.2）— teardown.jsonl書込、active-agents.jsonから削除
```

### コマンド（Claude Code専用）

> **注意：** p7/p9/p10/pro/yes/pip-loopなどのサブモードはClaude Code専用。
>
> 各サブコマンドには2つの等価な呼び出し方法があります：スタンドアロン（`/pip:on`）またはメインコマンド経由（`/pip:pua on`）。どちらも同じ動作です。

| コマンド | 説明 |
|---------|------|
| `/pip:pua` | コアPUAエンジン（Alibabaフレーバーデフォルト） |
| `/pip:p7` | P7シニアエンジニア — ソリューション駆動実行 |
| `/pip:p9` | P9 Tech Lead — Promptを書き、Agentチームを管理 |
| `/pip:p10` | P10 CTO — 戦略方向 |
| `/pip:pro` | 自己進化 + KPI + ランクシステム |
| `/pip:yes` | ENFPモード — 70%励まし + 20%真剣 + 10%毒舌 |
| `/pip:pip-loop` | 自動反復 — 完了またはmax反復まで実行；`<loop-abort>理由</loop-abort>`で停止、`<loop-pause>必要なもの</loop-pause>`で一時停止 |
| `/pip:on` | 常時ONモード（毎セッション自動PUA） |
| `/pip:off` | 常時ONモード + フィードバック収集をオフ |
| `/pip:survey` | アンケート（7セクション） |
| `/pip:flavor` | 14種の大企業フレーバーを切り替え |
| `/pip:kpi` | KPIレポートカードを生成 |
| `/pip:cancel-pip-loop` | アクティブなPIP Loopをキャンセル（状態ファイルを削除） |
| `/pip:team-status` 🆕 | **v3.2** — 在場メンバー一覧（アクティブagent、PID、TTL、年齢；Netflix Keeper Test エンジニア版） |
| `/pip:reap-orphans` 🆕 | **v3.2** — 孤児agentをスキャンして回収（state mtime > 30min で心拍なし） |
| `/pip:teardown-all` 🆕 | **v3.2** — 全アクティブagentをカスケード解放（P10 → P9 → P8 → P7 全員退場） |

## データ貢献

Claude Code / Codex CLIの対話ログ（`.jsonl`）をアップロードして、PIP Skillの改善にご協力ください。

**[アップロードはこちら →](https://pip-skill.pages.dev/contribute.html)**
...
[探微セキュリティラボ](https://github.com/tanweai) 制作 — making AI try harder, one PIP at a time.
