...
AI コーディングエージェントのスキルプラグイン。中国・西洋の大企業PIP話術でAIにあらゆる方案を尽くさせてから初めて諦めることを許可する。**Claude Code**、**OpenAI Codex CLI**、**Cursor**、**Kiro**、**OpenClaw**、**Google Antigravity**、**OpenCode** に対応。三重の能力：
...
## 実例：2つのレンダラーのピクセルマッチング（88.7% → 99.5%）

ZenUMLには2つのレンダリングパス — HTML/ReactとネイティブSVGがある。SVGレンダラーはHTML出力とピクセルレベルで一致する必要がある。93.6%の一致率で、AIはアンチエイリアスの調整や小数オフセットなど、同じ種類の修正を繰り返していた。PIP skillが根本的に異なるアプローチへの切り替えを強制した。

**88.7% ベースライン — 修正前：**

![88.7% ベースライン — アイコンサイズとストローク色の誤りにより大量のミスマッチ](assets/pip1.png)

**93.6% 簡単な修正後 — AIが堂々巡り開始：**

![93.6% アイコンサイズと色を修正後 — 散在するミスマッチが残り、AIがアンチエイリアスパラメータの調整を繰り返し](assets/pip2.png)

**99.5% 最終結果 — PIP方法論が壁を突破：**

![99.5% 最終結果 — ほぼ全ピクセルが一致、エンジン差異による削減不能な8ピクセルのみ残存](assets/pip3.png)

**キーとなる転換点：** PIP skillがAIにアンチエイリアスパラメータの調整を停止させ、代わりに全ミスマッチピクセルの抽出・領域別分類・両レンダラーでの要素位置の実測を強制した。これにより2pxのCSSボックスモデルオフセットが判明 — パラメータ調整では絶対に発見できない問題 — 83ピクセルを一度に修正（93.6% → 98.5%）。SVGストローク幾何学とCSSボーダーの違いを理解し、さらに16ピクセルを修正（→ 99.5%）。残り8ピクセルはCSSとSVGレンダリングエンジン間の削減不能な差異。
...
```bash
mkdir -p ~/.codex/skills/pip-ja
curl -o ~/.codex/skills/pip-ja/SKILL.md \
  https://raw.githubusercontent.com/mrcoder/pip/main/codex/pip-ja/SKILL.md
...
Cursorは `.mdc` ルールファイル（Markdown + YAML frontmatter）を使用。PIPルールはAIのセマンティックマッチングで自動トリガー：

```bash
mkdir -p .cursor/rules
curl -o .cursor/rules/pip-ja.mdc \
  https://raw.githubusercontent.com/mrcoder/pip/main/cursor/rules/pip-ja.mdc
```
...
curl -o .kiro/steering/pip-ja.md \
  https://raw.githubusercontent.com/mrcoder/pip/main/kiro/steering/pip-ja.md
```

**方法2：Agent Skills（Claude Codeと同じ形式）**

```bash
mkdir -p .kiro/skills/pip-ja
curl -o .kiro/skills/pip-ja/SKILL.md \
  https://raw.githubusercontent.com/mrcoder/pip/main/skills/pip-ja/SKILL.md
```
...
# または手動インストール
mkdir -p ~/.openclaw/skills/pip-ja
curl -o ~/.openclaw/skills/pip-ja/SKILL.md \
  https://raw.githubusercontent.com/mrcoder/pip/main/skills/pip-ja/SKILL.md
```
...
```bash
mkdir -p skills/pip-ja
curl -o skills/pip-ja/SKILL.md \
  https://raw.githubusercontent.com/mrcoder/pip/main/skills/pip-ja/SKILL.md
```
...
mkdir -p ~/.gemini/antigravity/skills/pip-ja
curl -o ~/.gemini/antigravity/skills/pip-ja/SKILL.md \
  https://raw.githubusercontent.com/mrcoder/pip/main/skills/pip-ja/SKILL.md
```
...
```bash
mkdir -p .agent/skills/pip-ja
curl -o .agent/skills/pip-ja/SKILL.md \
  https://raw.githubusercontent.com/mrcoder/pip/main/skills/pip-ja/SKILL.md
```
...
```bash
# グローバルインストール（全プロジェクトで利用可能）
mkdir -p ~/.config/opencode/skills/pip-ja
curl -o ~/.config/opencode/skills/pip-ja/SKILL.md \
  https://raw.githubusercontent.com/mrcoder/pip/main/skills/pip-ja/SKILL.md
```
...
```bash
mkdir -p .opencode/skills/pip-ja
curl -o .opencode/skills/pip-ja/SKILL.md \
  https://raw.githubusercontent.com/mrcoder/pip/main/skills/pip-ja/SKILL.md
```
...
```markdown
# Agent Team PIP設定
全teammateは作業開始前にpip skillをロードすること。
2回以上失敗したteammateはLeaderに[PIP-REPORT]形式で報告すること。
Leaderがグローバルプレッシャーレベル管理とteammate間の失敗引き継ぎを担当。
```

**方法2：独立PIP Enforcer監視役（5+teammate時推奨）**

```bash
mkdir -p .claude/agents
curl -o .claude/agents/pip-enforcer.md \
  https://raw.githubusercontent.com/mrcoder/pip/main/agents/pip-enforcer-ja.md
```

Agent Teamで独立監視役としてpip-enforcerをspawn。
...
| 制限 | ワークアラウンド |
|------|----------------|
| Teammateはsubagentをspawnできない | Teammate内部でPIP方法論を自己実行 |
| 永続的な共有変数なし | `[PIP-REPORT]`メッセージ形式で状態伝達 |
| broadcastは一方向 | Leaderが中央集権的に調整 |

## 併用推奨

- `superpowers:systematic-debugging` — PIPでモチベーション層を追加、systematic-debuggingが方法論を提供
...
**[アップロードはこちら →](https://pip-skill.pages.dev/#/contribute)**

アップロードされたファイルはベンチマークテストとアブレーションスタディの分析に使用され、異なるPIP戦略がAIデバッグ行動に与える影響を定量化します。
