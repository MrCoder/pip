...
実際のデバッグシナリオ。agent-kms MCPサーバーのロードに失敗し、AIが同じ思考（プロトコル形式の変更、バージョン番号の推測）で堂々巡りを続けた後、ユーザーが手動で `/pip` をトリガー。

**L3 トリガー → 7項目チェックリスト強制実行：**

![PIP L3トリガー — 推測を停止し、体系的チェックリストを実行、MCPログから真のエラー情報を発見](assets/pip1.jpg)

**根本原因特定 → ログから登録メカニズムを追跡：**

![根本原因 — claude mcpが管理するサーバー登録方式は手動の.claude.json編集とは異なる](assets/pip2.jpg)

**振り返り → PIPの実際の効果：**

![対話の振り返り — PIP skillが堂々巡りを強制停止、体系的チェックリストが以前チェックしたことのなかったClaude Code MCPログディレクトリの発見を促した](assets/pip3.jpg)
...
curl -o ~/.codex/skills/pip-ja/SKILL.md \
  https://raw.githubusercontent.com/tanweai/pua/main/codex/pip-ja/SKILL.md
...
curl -o .cursor/rules/pip-ja.mdc \
  https://raw.githubusercontent.com/tanweai/pua/main/cursor/rules/pip-ja.mdc
```
...
curl -o .kiro/steering/pip-ja.md \
  https://raw.githubusercontent.com/tanweai/pua/main/kiro/steering/pip-ja.md
```
...
curl -o .kiro/skills/pip-ja/SKILL.md \
  https://raw.githubusercontent.com/tanweai/pua/main/skills/pip-ja/SKILL.md
```
...
curl -o .claude/agents/pip-enforcer.md \
  https://raw.githubusercontent.com/tanweai/pua/main/agents/pip-enforcer-ja.md
```
...
**[アップロードはこちら →](https://PLACEHOLDER_OPENPIP_DOMAIN/#/contribute)**
